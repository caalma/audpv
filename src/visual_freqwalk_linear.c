...
// Función interna para dibujar el visualizador freqwalklineal
void draw_freqwalk_linear(SDL_Renderer *renderer, const void *audio_data, size_t data_size, int channels, int format, int w, int h, int sampleRate, bool *running) {
    ...
    double freq_max = 20000.0;    // Frecuencia máxima (20 kHz)
    double freq_step = (freq_max - freq_min) / h; // Paso de frecuencia por píxel
    ...
    for (int y = 0; y < h; y++) {
        ...
        float gamma = 0.5; // Factor de ajuste (menor que 1 para amplificar intensidades bajas)
