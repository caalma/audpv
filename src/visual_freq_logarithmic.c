...
void draw_freq_logarithmic(SDL_Renderer *renderer, const void *audio_data, size_t data_size, int channels, int format, int w, int h, int sampleRate, bool *running) {
    ...
    for (int i = 0; i < w; i++) {
        double log_pos = pow(10, (double)i / w * log10(samples_per_channel / 2));
        int freq_index = (int)log_pos;
        if (freq_index >= samples_per_channel / 2) freq_index = samples_per_channel / 2 - 1;
